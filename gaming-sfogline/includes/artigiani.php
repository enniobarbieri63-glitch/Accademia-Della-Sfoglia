<?php
/**
 * artigiani.php — "Gli Artigiani della Pasta": vetrine di partner paganti
 * (pastifici, botteghe, piccoli produttori), a pagamento tramite bonifico.
 *
 * CPT gs_artigiano (privato come gli altri CPT del progetto: la visibilità
 * pubblica la decide questo file, non WordPress):
 *  - post_title   = nome dell'attività
 *  - post_name    = slug, usato nell'indirizzo pubblico (?gs_art=slug)
 *  - post_author  = il partner (account WordPress creato dal titolare)
 *  - meta gs_art_comune            = comune/città (testo)
 *  - meta gs_art_logo              = URL del logo
 *  - meta gs_art_testo             = racconto libero ("Chi siamo")
 *  - meta gs_art_media             = [ {url} ] galleria foto (max 6)
 *  - meta gs_art_youtube           = URL del video di presentazione
 *  - meta gs_art_indirizzo         = indirizzo (facoltativo: se vuoto, niente
 *                                    riquadro "Dove trovarci" nella vetrina)
 *  - meta gs_art_email_contatto    = dove arrivano i messaggi del modulo di contatto
 *  - meta gs_art_stato             = 'vuota' | 'in_attesa' | 'approvata' | 'rifiutata' | 'sospesa'
 *  - meta gs_art_scadenza          = 'AAAA-MM-GG' fino a quando l'abbonamento è pagato
 *  - meta gs_art_pagamenti         = [ {data, importo, note} ] storico bonifici (mai sovrascritto, solo aggiunto)
 *
 * Account del partner: stesso schema già usato per i "lettori" (vedi
 * letture.php) — meta utente gs_status = 'artigiano' invece di 'in_attesa'/
 * 'approvata': gs_is_approved() considera "sfoglina approvata" solo lo stato
 * vuoto o 'approvata', quindi un account 'artigiano' resta escluso da solo da
 * tutto il resto del gaming (Le Sfogline, sfide, corsi…) senza dover toccare
 * gli altri moduli.
 *
 * Visibilità pubblica di una vetrina: SOLO se approvata E con abbonamento in
 * corso (gs_art_scadenza non passata). Alla scadenza la vetrina si nasconde
 ...
 * registra un nuovo bonifico con una nuova scadenza.
 */
...
// CPT
// -----------------------------------------------------------------------------
add_action( 'init', 'gs_art_register_cpt' );
function gs_art_register_cpt() {
	register_post_type( 'gs_artigiano', array(
		'labels'       => array(
			'name'          => 'Artigiani della Pasta',
			'singular_name' => 'Artigiano della Pasta',
		),
		'public'       => false,
		'show_ui'      => false,
		'show_in_menu' => false,
...
// Indirizzo pubblico della vetrina singola: /gli-artigiani-della-pasta/?gs_art=slug
add_action( 'init', function () {
	add_rewrite_tag( '%gs_art%', '([^&]+)' );
} );
...
/** Dati normalizzati di un artigiano, pronti per l'uso nei template. */
function gs_art_get( $id ) {
	$id = (int) $id;
	$media = get_post_meta( $id, 'gs_art_media', true );
	$pag   = get_post_meta( $id, 'gs_art_pagamenti', true );
	return array(
		'id'        => $id,
		'nome'      => get_the_title( $id ),
		'slug'      => get_post_field( 'post_name', $id ),
		'autore'    => (int) get_post_field( 'post_author', $id ),
		'comune'    => (string) get_post_meta( $id, 'gs_art_comune', true ),
		'logo'      => (string) get_post_meta( $id, 'gs_art_logo', true ),
		'testo'     => (string) get_post_meta( $id, 'gs_art_testo', true ),
		'media'     => is_array( $media ) ? $media : array(),
		'youtube'   => (string) get_post_meta( $id, 'gs_art_youtube', true ),
		'indirizzo' => (string) get_post_meta( $id, 'gs_art_indirizzo', true ),
		'email'     => (string) get_post_meta( $id, 'gs_art_email_contatto', true ),
		'stato'     => get_post_meta( $id, 'gs_art_stato', true ) ?: 'vuota',
		'scadenza'  => (string) get_post_meta( $id, 'gs_art_scadenza', true ),
		'pagamenti' => is_array( $pag ) ? $pag : array(),
	);
}

/** True se l'abbonamento è in corso (scadenza impostata e non passata). */
function gs_art_attivo( $id ) {
	$scadenza = get_post_meta( (int) $id, 'gs_art_scadenza', true );
	if ( ! $scadenza ) {
		return false;
	}
...
function gs_art_pubblicata( $id ) {
	$id = (int) $id;
	return 'approvata' === get_post_meta( $id, 'gs_art_stato', true ) && gs_art_attivo( $id );
}

/**
 * Il post gs_artigiano posseduto da un utente (uno solo per account), o null.
 *
 ...
 * Verificato il 25/08/2026 confrontando l'SQL generato con/senza quel
 * parametro. Si filtra in PHP sull'elenco già letto da gs_art_elenco(),
 * poche decine di righe al massimo: costa niente ed è sicuro.
 */
function gs_art_owner_post( $uid ) {
	$uid = (int) $uid;
	if ( $uid < 1 ) {
		return null;
	}
...
function gs_art_is_artigiano( $uid ) {
	return null !== gs_art_owner_post( $uid );
}

/** Tutti gli artigiani (per il pannello di amministrazione), non cestinati. */
function gs_art_elenco( $status = 'publish' ) {
...
/** URL della vetrina pubblica di un artigiano. */
function gs_art_url( $id ) {
	$slug    = get_post_field( 'post_name', (int) $id );
	$page_id = (int) get_option( 'gs_page_artigiani' );
	if ( ! $slug || ! $page_id ) {
		return '';
	}
	return add_query_arg( 'gs_art', $slug, get_permalink( $page_id ) );
}

/** URL del pannello di autogestione del partner. */
function gs_art_pannello_url() {
	return function_exists( 'gs_pagina_url' ) ? gs_pagina_url( 'gs_page_artigiano_pannello' ) : home_url( '/' );
}

/** Estrae l'ID video da un URL YouTube (watch, youtu.be, shorts, embed). */
function gs_art_youtube_id( $url ) {
	$url = trim( (string) $url );
	if ( '' === $url ) {
		return '';
	}
...
function gs_art_stato_label( $stato ) {
	$labels = array(
...
// -----------------------------------------------------------------------------
// Shortcode pubblico [gs_artigiani] — direttorio + vetrina singola
// -----------------------------------------------------------------------------
add_shortcode( 'gs_artigiani', 'gs_sc_artigiani' );
function gs_sc_artigiani() {
	$slug = get_query_var( 'gs_art' );
	if ( ! $slug && isset( $_GET['gs_art'] ) ) {
		$slug = sanitize_title( wp_unslash( $_GET['gs_art'] ) );
	}
	if ( $slug ) {
		return gs_art_render_vetrina_da_slug( $slug );
	}
	return gs_art_render_direttorio();
}

function gs_art_render_vetrina_da_slug( $slug ) {
	...
	if ( ! $posts || ! gs_art_pubblicata( $posts[0]->ID ) ) {
		return gs_art_render_non_disponibile();
	}
	return gs_art_render_vetrina( gs_art_get( $posts[0]->ID ) );
}

function gs_art_render_non_disponibile() {
	$dir = function_exists( 'gs_pagina_url' ) ? gs_pagina_url( 'gs_page_artigiani' ) : home_url( '/' );
	return '<div class="gs-art"><div class="gs-art-notice">'
		. '<p>Questa vetrina non è al momento disponibile.</p>'
		. '<p><a class="gs-art-btn" href="' . esc_url( $dir ) . '">← Torna a Gli Artigiani della Pasta</a></p>'
		. '</div></div>';
}

/** Direttorio pubblico: tutti gli artigiani con vetrina attiva. */
function gs_art_render_direttorio() {
	$tutti = gs_art_elenco();
	$attivi = array_filter( $tutti, function ( $p ) { return gs_art_pubblicata( $p->ID ); } );
	...
	<div class="gs-art">
		<div class="gs-art-hero">
			<div class="gs-art-hero-testo">
				...
				<h1>Gli Artigiani della Pasta</h1>
				<p>Pastifici, botteghe e piccoli produttori che portano avanti ogni giorno la tradizione della sfoglia fatta a mano. Ogni vetrina è raccontata e curata direttamente da chi lavora la pasta.</p>
			</div>
			<?php echo gs_art_onde_html(); ?>
		</div>
			...
			<?php else : ?>
				<div class="gs-art-griglia">
					<?php foreach ( $attivi as $p ) :
						...
						?>
						<a class="gs-art-card" href="<?php echo esc_url( gs_art_url( $a['id'] ) ); ?>">
...
/** Vetrina pubblica di un singolo artigiano. */
function gs_art_render_vetrina( $a ) {
	$url = gs_art_url( $a['id'] );
	ob_start();
	?>
	<div class="gs-art">
		<div class="gs-art-hero">
			<div class="gs-art-hero-testo">
				<div class="gs-art-eyebrow">Gli Artigiani della Pasta<?php echo $a['comune'] ? ' — ' . esc_html( $a['comune'] ) : ''; ?></div>
		...
		<div class="gs-art-wrap gs-art-condividi">
			<?php echo gs_art_condividi_html( $url, $a['nome'] ); ?>
		</div>
		<?php endif; ?>
		...
		<div class="gs-art-wrap gs-art-galleria">
			<div class="gs-art-eyebrow gs-art-centro">La bottega in foto</div>
		...
		<?php $yt = gs_art_youtube_id( $a['youtube'] ); if ( $yt ) : ?>
		...
		<div class="gs-art-wrap gs-art-doppie">
			<div class="gs-art-doppie-grid">
				...
				<div class="gs-art-cardgrande">
					...
					<form class="gs-art-form-contatto" data-artigiano="<?php echo (int) $a['id']; ?>" onsubmit="return false">
						...
						<button type="submit" class="gs-art-btn gs-art-contatta-invia">Invia messaggio</button>
						<span class="gs-art-form-msg"></span>
					</form>
				</div>
				<?php endif; ?>
			</div>
 ...
 */
function gs_art_condividi_html( $url, $nome ) {
	$testo_wa = $nome . ' — ' . $url;
	...
	$out .= '<button type="button" class="gs-art-condividi-link gs-art-copia-link" data-link="' . esc_attr( $url ) . '">🔗 Copia link</button>';
	$out .= '</div>';
	return $out;
}
...
// -----------------------------------------------------------------------------
add_action( 'wp_ajax_nopriv_gs_art_contatta', 'gs_ajax_art_contatta' );
add_action( 'wp_ajax_gs_art_contatta', 'gs_ajax_art_contatta' );
function gs_ajax_art_contatta() {
	...
	$id  = isset( $_POST['artigiano'] ) ? (int) $_POST['artigiano'] : 0;
	if ( 'gs_artigiano' !== get_post_type( $id ) || ! gs_art_pubblicata( $id ) ) {
		wp_send_json_error( array( 'message' => 'Vetrina non valida.' ) );
	}
	$dest = get_post_meta( $id, 'gs_art_email_contatto', true );
	...
	$corpo = "Nuovo messaggio dalla vetrina \"$nome_attivita\" (Gli Artigiani della Pasta):\n\n"
		. "Da: $nome <$email>\n\n$messaggio";
	...
	wp_send_json_success( array( 'message' => 'Messaggio inviato! Riceverai risposta direttamente dall\'artigiano.' ) );
}

// -----------------------------------------------------------------------------
// Pannello di autogestione del partner — shortcode [gs_vetrina_artigiano_pannello]
// -----------------------------------------------------------------------------
add_shortcode( 'gs_vetrina_artigiano_pannello', 'gs_sc_art_pannello' );
function gs_sc_art_pannello() {
	if ( $g = gs_gate_riservato() ) { return $g; }
	$post = gs_art_owner_post( get_current_user_id() );
	if ( ! $post ) {
		return gs_box_open( 'Area riservata', 'gs-notice' ) . '<p>Questa pagina è riservata ai partner de Gli Artigiani della Pasta. Se pensi si tratti di un errore, scrivi all\'Accademia.</p>' . gs_box_close();
	}
	$a = gs_art_get( $post->ID );

	$out  = gs_box_open( '🍝 La Mia Vetrina — ' . $a['nome'], '', 'gs-box-art-pannello' );
	$out .= '<p><span class="gs-pill">' . esc_html( gs_art_stato_label( $a['stato'] ) ) . '</span>'
		. ( gs_art_attivo( $a['id'] ) ? ' <span class="gs-pill gs-pill-verde">Abbonamento attivo fino al ' . esc_html( gs_data_it( $a['scadenza'] ) ) . '</span>' : ' <span class="gs-pill gs-pill-rosso">Abbonamento non attivo</span>' ) . '</p>';
	...
	if ( gs_art_pubblicata( $a['id'] ) ) {
		$url = gs_art_url( $a['id'] );
		$out .= '<div class="gs-todo-riquadro">';
		$out .= '<p><strong>La tua vetrina è online:</strong> puoi condividerla con i tuoi clienti sui social, o incollare il link dove preferisci.</p>';
		$out .= '<p><input type="text" class="gs-art-link-input" value="' . esc_attr( $url ) . '" readonly onclick="this.select()" style="width:100%;margin-bottom:8px"></p>';
		$out .= gs_art_condividi_html( $url, $a['nome'] );
		$out .= '</div>';
	} else {
	...
	if ( $a['logo'] ) {
		...
		$out .= '<button class="gs-btn gs-btn-sm gs-btn-ghost gs-art-logo-rimuovi">Rimuovi logo</button>';
	} else {
	...
	if ( $a['media'] ) {
		...
		foreach ( $a['media'] as $i => $m ) {
			$out .= '<div class="gs-bio-media-item">' . gs_msg_media_html( $m['url'], 'image' )
				. '<button class="gs-btn gs-btn-sm gs-btn-ghost gs-art-media-rimuovi" data-i="' . (int) $i . '">Rimuovi</button></div>';
		}
		$out .= '</div>';
	}

	$out .= '<form class="gs-form gs-form-art" onsubmit="return false">';
	$out .= '<p><label>Logo dell\'attività (facoltativo)<br><input type="file" name="logo" class="gs-art-logo-file" accept="image/*"></label></p>';
	...
	$out .= '<p><label>Aggiungi una foto alla galleria (fino a 6 in tutto, ridotte automaticamente)<br><input type="file" name="foto" class="gs-art-foto-file" accept="image/*"></label></p>';
	...
	$out .= '<p><button class="gs-btn gs-art-salva">Salva e invia per approvazione</button> <span class="gs-art-msg gs-richiesta-esito"></span></p>';
	$out .= '</form>';
...
/** Data italiana leggibile da 'AAAA-MM-GG'. */
function gs_data_it( $ymd ) {
	if ( ! preg_match( '/^\d{4}-\d{2}-\d{2}$/', (string) $ymd ) ) {
		return $ymd;
	}
	return date_i18n( 'd/m/Y', strtotime( $ymd ) );
}

add_action( 'wp_ajax_gs_art_salva', 'gs_ajax_art_salva' );
function gs_ajax_art_salva() {
	check_ajax_referer( 'gs_ajax', 'nonce' );
	$uid = get_current_user_id();
	if ( ! $uid || ! gs_puo_partecipare( $uid ) ) { wp_send_json_error( array( 'message' => 'Il tuo account non può fare questa cosa adesso.' ) ); }
	$post = gs_art_owner_post( $uid );
	...
	update_post_meta( $id, 'gs_art_comune', $comune );
	update_post_meta( $id, 'gs_art_testo', $testo );
	update_post_meta( $id, 'gs_art_youtube', $youtube );
	update_post_meta( $id, 'gs_art_indirizzo', $indirizzo );
	update_post_meta( $id, 'gs_art_email_contatto', $email );
	...
	if ( function_exists( 'gs_msg_upload' ) ) {
		...
		if ( is_array( $logo ) ) {
			...
			update_post_meta( $id, 'gs_art_logo', $logo['url'] );
		}
		...
		if ( is_array( $foto ) ) {
			...
			update_post_meta( $id, 'gs_art_media', $media );
		}
	}
	...
	if ( function_exists( 'gs_inbox_crea' ) ) {
		gs_inbox_crea(
			'Vetrina Artigiani da approvare: ' . get_the_title( $id ),
			'Un partner di "Gli Artigiani della Pasta" ha inviato o modificato la propria vetrina. Va approvata dalla Plancia Generale per tornare visibile nella sezione pubblica.',
			array( 'author' => $uid, 'link_ancora' => 'admin.php?page=gs-generale#gs-zona-artigiani' )
		);
	}
	wp_send_json_success( array( 'message' => 'Vetrina salvata e inviata per approvazione.' ) );
}

add_action( 'wp_ajax_gs_art_logo_rimuovi', 'gs_ajax_art_logo_rimuovi' );
function gs_ajax_art_logo_rimuovi() {
	check_ajax_referer( 'gs_ajax', 'nonce' );
	$post = gs_art_owner_post( get_current_user_id() );
	if ( ! $post ) { wp_send_json_error( array( 'message' => 'Nessuna vetrina collegata a questo account.' ) ); }
	delete_post_meta( $post->ID, 'gs_art_logo' );
	update_post_meta( $post->ID, 'gs_art_stato', 'in_attesa' );
	wp_send_json_success( array( 'message' => 'Logo rimosso.' ) );
}

add_action( 'wp_ajax_gs_art_media_rimuovi', 'gs_ajax_art_media_rimuovi' );
function gs_ajax_art_media_rimuovi() {
	check_ajax_referer( 'gs_ajax', 'nonce' );
	$post = gs_art_owner_post( get_current_user_id() );
	if ( ! $post ) { wp_send_json_error( array( 'message' => 'Nessuna vetrina collegata a questo account.' ) ); }
...
function gs_pannello_artigiani() {
	...
	echo '<p><button class="gs-btn gs-btn-sm gs-art-crea-invia">Crea account e vetrina</button> <span class="gs-art-crea-msg gs-richiesta-esito"></span></p>';
	echo '</form>';

	$tutti = gs_art_elenco();
	echo '<h4 style="margin-top:18px">Artigiani (' . count( $tutti ) . ')</h4>';
	if ( ! $tutti ) {
		echo '<p class="gs-hint">Nessun artigiano ancora creato.</p>';
	} else {
		echo '<div class="gs-inbox-lista gs-paginate" data-per-page="8">';
		foreach ( $tutti as $p ) {
			$a = gs_art_get( $p->ID );
			$u = get_userdata( $a['autore'] );
			$attivo = gs_art_attivo( $a['id'] );
			echo '<details class="gs-inbox-item' . ( 'in_attesa' === $a['stato'] ? ' gs-non-letto gs-attesa-forte' : '' ) . '" data-art="' . (int) $a['id'] . '">';
			echo '<summary class="gs-inbox-oggetto">' . ( 'in_attesa' === $a['stato'] ? '<span class="gs-dot"></span> ' : '' ) . esc_html( $a['nome'] )
				. ( $a['comune'] ? ' — ' . esc_html( $a['comune'] ) : '' )
				. ' <span class="gs-msg-data">' . esc_html( gs_art_stato_label( $a['stato'] ) ) . ( $attivo ? ' · attivo fino al ' . esc_html( gs_data_it( $a['scadenza'] ) ) : ' · abbonamento non attivo' ) . '</span></summary>';
			...
			if ( gs_art_pubblicata( $a['id'] ) ) {
				echo '<p><a class="gs-btn gs-btn-sm gs-btn-ghost" href="' . esc_url( gs_art_url( $a['id'] ) ) . '" target="_blank" rel="noopener">Vedi la vetrina pubblica</a></p>';
			}

			echo '<p>';
			if ( 'approvata' !== $a['stato'] ) {
				echo '<button class="gs-btn gs-btn-sm gs-art-modera" data-art="' . (int) $a['id'] . '" data-esito="approvata">Approva</button> ';
			}
			if ( 'sospesa' !== $a['stato'] && 'vuota' !== $a['stato'] ) {
				echo '<button class="gs-btn gs-btn-sm gs-btn-ghost gs-art-modera" data-art="' . (int) $a['id'] . '" data-esito="sospesa">Sospendi</button> ';
			}
			echo '<button class="gs-btn gs-btn-sm gs-btn-ghost gs-art-modera" data-art="' . (int) $a['id'] . '" data-esito="rifiutata">Rifiuta</button> ';
			echo '<span class="gs-art-mod-msg gs-richiesta-esito"></span></p>';

			echo '<details class="gs-sezione-aiuto"><summary>💶 Registra un bonifico</summary>';
			echo '<form class="gs-form gs-form-art-pagamento" data-art="' . (int) $a['id'] . '" onsubmit="return false">';
			...
			echo '<p><button class="gs-btn gs-btn-sm gs-art-pagamento-invia">Registra pagamento</button> <span class="gs-art-pag-msg gs-richiesta-esito"></span></p>';
			echo '</form>';
			if ( $a['pagamenti'] ) {
			...
			echo '<p><button class="gs-btn gs-btn-sm gs-btn-ghost gs-art-cestina" data-art="' . (int) $a['id'] . '">🗑️ Sposta nel cestino</button></p>';
			echo '</div></details>';
		}
		echo '</div>';
	}

	$cestino = gs_art_elenco( 'trash' );
	echo '<details class="gs-todo-cestino"><summary>🗑️ Cestino Artigiani (' . count( $cestino ) . ')</summary>';
	if ( ! $cestino ) {
		echo '<p class="gs-hint">Il cestino è vuoto.</p>';
	} else {
		foreach ( $cestino as $p ) {
			echo '<p>' . esc_html( get_the_title( $p ) ) . ' <button class="gs-btn gs-btn-sm gs-art-ripristina" data-art="' . (int) $p->ID . '">↺ Ripristina</button></p>';
		}
	}
	echo '</details>';
}

add_action( 'wp_ajax_gs_art_crea', 'gs_ajax_art_crea' );
function gs_ajax_art_crea() {
	check_ajax_referer( 'gs_ajax', 'nonce' );
	...
	// l'8 agosto 2026 (agganciata l'email di una sfoglina già esistente).
	// Da qui in avanti si può fare, ma solo dicendo di chi si tratta e
	// facendolo confermare (trovato il 25/08/2026).
	...
	// Una vetrina per account: gs_art_owner_post() ne restituisce una sola,
	...
	if ( $comune ) {
		update_post_meta( $post_id, 'gs_art_comune', $comune );
	}
	update_post_meta( $post_id, 'gs_art_stato', 'vuota' );
...
add_action( 'wp_ajax_gs_art_modera', 'gs_ajax_art_modera' );
function gs_ajax_art_modera() {
	check_ajax_referer( 'gs_ajax', 'nonce' );
	if ( ! gs_can_manage() ) { wp_send_json_error( array( 'message' => 'Permesso negato.' ) ); }
	$id    = isset( $_POST['art'] ) ? (int) $_POST['art'] : 0;
	$esito = isset( $_POST['esito'] ) ? sanitize_key( wp_unslash( $_POST['esito'] ) ) : '';
	if ( 'gs_artigiano' !== get_post_type( $id ) || ! in_array( $esito, array( 'approvata', 'rifiutata', 'sospesa' ), true ) ) {
		wp_send_json_error( array( 'message' => 'Dati non validi.' ) );
	}
	update_post_meta( $id, 'gs_art_stato', $esito );

	$autore = (int) get_post_field( 'post_author', $id );
	$u      = $autore ? get_userdata( $autore ) : null;
	if ( $u && $u->user_email ) {
		$msg = array(
			'approvata' => "Ciao " . $u->display_name . ",\n\nla tua vetrina \"" . get_the_title( $id ) . "\" è stata approvata. Se anche l'abbonamento è attivo, è già visibile in \"Gli Artigiani della Pasta\".",
	...
	wp_send_json_success( array( 'message' => 'Stato aggiornato: ' . gs_art_stato_label( $esito ) . '.' ) );
}

add_action( 'wp_ajax_gs_art_pagamento', 'gs_ajax_art_pagamento' );
function gs_ajax_art_pagamento() {
	check_ajax_referer( 'gs_ajax', 'nonce' );
	if ( ! gs_can_manage() ) { wp_send_json_error( array( 'message' => 'Permesso negato.' ) ); }
	$id       = isset( $_POST['art'] ) ? (int) $_POST['art'] : 0;
	...
	if ( 'gs_artigiano' !== get_post_type( $id ) ) { wp_send_json_error( array( 'message' => 'Artigiano non valido.' ) ); }
	...
	$log = get_post_meta( $id, 'gs_art_pagamenti', true );
	if ( ! is_array( $log ) ) { $log = array(); }

	// Blocca il doppio invio, stesso schema già usato per i pagamenti del
	// calendario (C1) e l'accredito dei token (T1): l'identificativo arriva
	// dalla scheda, si controlla e si scrive PRIMA di toccare il registro —
	// ma DOPO i controlli sopra, altrimenti una richiesta respinta per la
	// scadenza segnerebbe il rif come "già visto" e il reinvio confermato
	// dallo stesso clic verrebbe rifiutato per errore. Il registro è per
	// vetrina, non per persona: qui non serve il tetto di 50 messo sui
	// token, un partner registra uno o due bonifici l'anno.
	...
	if ( $rif ) {
		...
		// 15 secondi del calendario: un partner che versa due volte lo
...
add_action( 'wp_ajax_gs_art_cestina', 'gs_ajax_art_cestina' );
function gs_ajax_art_cestina() {
	check_ajax_referer( 'gs_ajax', 'nonce' );
	if ( ! gs_can_manage() ) { wp_send_json_error( array( 'message' => 'Permesso negato.' ) ); }
	$id = isset( $_POST['art'] ) ? (int) $_POST['art'] : 0;
	if ( 'gs_artigiano' !== get_post_type( $id ) ) { wp_send_json_error( array( 'message' => 'Artigiano non valido.' ) ); }

	// Se l'account collegato era stato reso "artigiano" da questa vetrina (es.
	...
	if ( $autore && 'artigiano' === get_user_meta( $autore, 'gs_status', true ) ) {
...
add_action( 'wp_ajax_gs_art_ripristina', 'gs_ajax_art_ripristina' );
function gs_ajax_art_ripristina() {
	check_ajax_referer( 'gs_ajax', 'nonce' );
	if ( ! gs_can_manage() ) { wp_send_json_error( array( 'message' => 'Permesso negato.' ) ); }
	$id = isset( $_POST['art'] ) ? (int) $_POST['art'] : 0;
	if ( 'gs_artigiano' !== get_post_type( $id ) ) { wp_send_json_error( array( 'message' => 'Artigiano non valido.' ) ); }
	...
	// Simmetrico a gs_ajax_art_cestina(): se al momento del cestino l'account
	// era tornato sfoglina normale, ripristinando la vetrina torna "artigiano".
	$autore = (int) get_post_field( 'post_author', $id );
	if ( $autore && 'artigiano' !== get_user_meta( $autore, 'gs_status', true ) ) {
		update_user_meta( $autore, 'gs_status', 'artigiano' );
	}
...
// pannelli generali) — stesso schema di gs_abbonamento_controlla_scadenze()
// in abbonamenti.php, gira sul cron giornaliero già esistente del plugin.
// Richiesto da Ennio l'11/08/2026: prima l'abbonamento scaduto nascondeva la
// vetrina in silenzio, senza avvisare nessuno.
// -----------------------------------------------------------------------------
add_action( 'gs_daily_cron', 'gs_art_controlla_scadenze' );
function gs_art_controlla_scadenze() {
	$giorni_preavviso = 7;
	$oggi = current_time( 'timestamp' );
	foreach ( gs_art_elenco() as $p ) {
		$scadenza = get_post_meta( $p->ID, 'gs_art_scadenza', true );
		...
		// vetrine già avvisate col codice precedente, gs_art_avviso_per vale
		...
		$testi = array(
			'preavviso' => array(
				...
				'corpo_mail'   => "l'abbonamento della tua vetrina \"" . get_the_title( $p->ID ) . "\" su \"Gli Artigiani della Pasta\" scade il " . date_i18n( 'j F Y', $ts ) . ".\n\nPer restare visibile nella sezione pubblica, rinnova con un bonifico prima di questa data: scrivi all'Accademia per i dati e la causale.",
				'inbox'        => false,
			),
			'ultimo' => array(
				'oggetto_mail' => "La tua vetrina scade domani — Accademia della Sfoglia",
				'corpo_mail'   => "l'abbonamento della tua vetrina \"" . get_the_title( $p->ID ) . "\" su \"Gli Artigiani della Pasta\" scade il " . date_i18n( 'j F Y', $ts ) . ": da domani, se non rinnovi, non sarà più visibile nella sezione pubblica.\n\nRinnova con un bonifico: scrivi all'Accademia per i dati e la causale.",
				'inbox'        => true,
			),
			'scaduto' => array(
				'oggetto_mail' => "La tua vetrina non è più visibile — Accademia della Sfoglia",
				'corpo_mail'   => "l'abbonamento della tua vetrina \"" . get_the_title( $p->ID ) . "\" su \"Gli Artigiani della Pasta\" è scaduto: la vetrina non è più visibile nella sezione pubblica.\n\nSi riaccende non appena registriamo il rinnovo: scrivi all'Accademia per i dati del bonifico.",
				'inbox'        => true,
			),
		);
		$t = $testi[ $fase ];
		...
		if ( $t['inbox'] && function_exists( 'gs_inbox_crea' ) ) {
			gs_inbox_crea(
				( 'scaduto' === $fase ? 'Vetrina scaduta' : 'Abbonamento in scadenza domani' ) . ' — Artigiani della Pasta: ' . get_the_title( $p->ID ),
				'scaduto' === $fase
					? 'Scaduto il ' . date_i18n( 'j/m/Y', $ts ) . ', vetrina non più visibile. Registra un nuovo bonifico dalla zona "Artigiani della Pasta" per riaccenderla.'
					: 'Scade domani, ' . date_i18n( 'j/m/Y', $ts ) . '. Registra un nuovo bonifico dalla zona "Artigiani della Pasta" per rinnovarlo, altrimenti la vetrina si nasconderà da sola dalla sezione pubblica.',
				array( 'author' => $autore )
			);
		}
	}
